...
def cashback(data: list[dict], year: int, month: int) -> dict:
    ...
    res = {}
    for item in data:
        date_transaction = datetime.strptime(item.get("Дата операции"), "%d.%m.%Y %H:%M:%S")
...
def search(query: str, data: list[dict]) -> list[dict]:
    result = []
    for item in data:
        if re.search(query, str(item.get('Описание'))) or re.findall(query, str(item.get('Категория'))):
            result.append(item)
    return result
